...
class Keyboard {
  ...
  computeGridTemplateColumns(wapper) {
    return "grid-template-columns:" + wapper.map(
      (w) => w.size != void 0 ? typeof w.size == "string" ? w.size : `${BASE_WIDTH * w.size}em` : `${BASE_WIDTH}em`
    ).join(" ");
  }
  // 展示预选字体部分
...
const _SnowKeyboard = class {
  constructor({ inputEl, theme, mode, type, size }) {
    ...
    __publicField(this, "focusHandler", (e) => {
      ...
      const { target } = e, { type, id } = target;
      if (type == "text" && (id == null ? void 0 : id.indexOf("keyboard-input")) >= 0) {
        this.keyboard.toggleKeyboardShow(true);
        (_a = _SnowKeyboard.inputEl) == null ? void 0 : _a.addEventListener("blur", this.blurHandler);
      }
    });
    // focus事件
